...
/**
 * Implementation of {@link Technique} that provides an embedded DSL.
 */
public class EDSLTechnique implements Technique {
    private Scene scene;
    ...
    protected Runnable withShader(final ShaderProgram shader, final Runnable drawing) {
        final DrawingState state = scene.context().state();
        return new Runnable() {
            @Override
            public void run() {
